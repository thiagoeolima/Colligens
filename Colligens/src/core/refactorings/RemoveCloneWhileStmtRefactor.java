...
public class RemoveCloneWhileStmtRefactor implements Visitor{
	...
	public String getWhileStmtCondition(WhileStatement whileStmt){
		String condition = "";
		
		if (whileStmt.getChildren().size() > 0){
			Node naryExpr = whileStmt.getChildren().get(0);
			
			
			if (naryExpr instanceof NAryExpr){
				VisitorPrinterToString printerStr = new VisitorPrinterToString();
				naryExpr.accept(printerStr);
	...
	public void run(WhileStatement node) {
		...
		if (parent instanceof Opt){
			...
			String condition = this.getWhileStmtCondition(node);
			...
			while (index < (grandParent.getChildren().size() - 1)){
				...
				if (grandParent.getChildren().get(index) instanceof Opt){
					// Checking the While statements with the same body..
					if (grandParent.getChildren().get(index).getChildren().get(0) instanceof WhileStatement && !condition.equals(this.getWhileStmtCondition( ((WhileStatement) grandParent.getChildren().get(index).getChildren().get(0)) ))){
			...
			// Getting the condition of the while statements.. We must have at least two opt with the different conditions..
			if (count > 1){
				
				Node whileStmtClone = null;
				
				// While statement wrapper..
				Node whileStmtCloneAux = null;
				...
				for (int i = 0; i < count; i++){
					...
					whileStmtClone = optClone.getChildren().get(0);
					
					String cond = "";
					if (whileStmtClone instanceof WhileStatement){
						whileStmtCloneAux = refactor.cloneNode(whileStmtClone);
						cond = this.getWhileStmtCondition( (WhileStatement) whileStmtClone);
						
					// While statement wrapper..
					} else if (whileStmtClone instanceof CompoundStatement){
						cond = "1";
					}
				...
				// Adding the test condition to the while statement..
				if (whileStmtCloneAux.getChildren().size() > 0){
					Node naryExpr = whileStmtCloneAux.getChildren().get(0);
					
					if (naryExpr instanceof NAryExpr){
						naryExpr.getParent().getChildren().remove(0);
						
						Id id = new Id();
						id.setName(" ( test ) ");
						id.setParent(naryExpr.getParent());
						naryExpr.getParent().getChildren().add(0, id);
						grandParent.getChildren().add(firstOptNodeIndex, whileStmtCloneAux);
						whileStmtCloneAux.setParent(grandParent);
					}
				}
			}
		}

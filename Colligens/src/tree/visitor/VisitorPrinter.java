...
public class VisitorPrinter implements Visitor {
	...
	public VisitorPrinter(boolean printToFile) {
		if (printToFile) {
			...
			File file = new File(RefactoringFrontend.outputFilePath);
	...
	public void run(ExprList node) {
		...
		if (!optionalNodes) {
			...
			for (int i = 0; i < node.getChildren().size(); i++) {
				if (node.getChildren().get(i) instanceof Opt) {
					if (i >= 1
							&& node.getChildren().get(i).getChildren().size() > 0) {
						Node id = node.getChildren().get(i).getChildren()
								.get(0);
						if (id instanceof Id) {
		...
		/*
		 * int index = 1; List<FeatureExpr> conditions = new
		 * ArrayList<FeatureExpr>();
		 * 
		 * for (int i = 0; i < node.getChildren().size(); i++){ if
		 * (node.getChildren().get(i) instanceof Opt){ FeatureExpr expr =
		 * ((Opt)node.getChildren().get(i)).getConditional(); boolean add =
		 * true; for (FeatureExpr fexpr : conditions){ if
		 * (fexpr.or(expr).isTautology()){ add = false; break; } } if (add){
		 * conditions.add(expr); } } }
		 * 
	...
	public void run(Opt node) {
		...
		if (node.getChildren().size() > 0
				&& !node.getConditional().toString().trim().equals("False")) {
	...
	public void run(Declaration node) {
		...
		if (!(node.getParent() instanceof DeclarationStatement)
				&& !(node.getParent() instanceof DefineDirective)) {
			System.out.println(";");
		}
	}

...
public class TypeGeneratorVisitor implements Visitor {
	...
	public void run(Declaration node) {
		...
		while (child instanceof Opt) {
			child = child.getChildren().get(0);
		}

		if (child instanceof TypedefSpecifier
				|| child instanceof StructOrUnionSpecifier) {
			Type type = new Type();
			VisitorPrinterToString visitorPrinterToString = new VisitorPrinterToString();
			// TODO // node.accept(visitorPrinterToString);
			node.getParent().accept(visitorPrinterToString);

			String source = visitorPrinterToString.getSourceCode();
			type.setSource(source);

			type.setPresenceCondition(node.getPresenceCondition());
			this.types.add(type);
		}
	}
